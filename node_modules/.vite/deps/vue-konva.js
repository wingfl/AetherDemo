...
var Node = class _Node {
  ...
  _getTransform() {
    ...
    m.reset();
    const x = this.x(), y2 = this.y(), rotation = Konva.getAngle(this.rotation()), scaleX = (_a2 = this.attrs.scaleX) !== null && _a2 !== void 0 ? _a2 : 1, scaleY = (_b = this.attrs.scaleY) !== null && _b !== void 0 ? _b : 1, skewX = this.attrs.skewX || 0, skewY = this.attrs.skewY || 0, offsetX = this.attrs.offsetX || 0, offsetY = this.attrs.offsetY || 0;
    if (x !== 0 || y2 !== 0) {
      m.translate(x, y2);
    }
    if (rotation !== 0) {
  ...
  _toKonvaCanvas(config) {
    ...
    const stage = this.getStage(), x = config.x !== void 0 ? config.x : Math.floor(box.x), y2 = config.y !== void 0 ? config.y : Math.floor(box.y), pixelRatio = config.pixelRatio || 1, canvas = new SceneCanvas({
...
var Stage = class extends Container {
  ...
  setPointersPositions(evt) {
    ...
    if (evt.touches !== void 0) {
      ...
      this._pointerPositions = [{ x, y: y2, id: Util._getFirstPointerId(evt) }];
      this._changedPointerPositions = [
        { x, y: y2, id: Util._getFirstPointerId(evt) }
      ];
    }
  }
...
var Easings = {
  ...
  BounceEaseIn(t, b2, c, d) {
    return c - Easings.BounceEaseOut(d - t, 0, c, d) + b2;
  },
  BounceEaseInOut(t, b2, c, d) {
    if (t < d / 2) {
      return Easings.BounceEaseIn(t * 2, 0, c, d) * 0.5 + b2;
    } else {
      return Easings.BounceEaseOut(t * 2 - d, 0, c, d) * 0.5 + c * 0.5 + b2;
    }
  },
  EaseIn(t, b2, c, d) {
    return c * (t /= d) * t + b2;
...
var Path = class _Path extends Shape {
    ...
    switch (cp.command) {
      ...
      case "C":
        return _Path.getPointOnCubicBezier(t2length(length, _Path.getPathLength(dataArray), (i3) => {
          return getCubicArcLength([cp.start.x, p[0], p[2], p[4]], [cp.start.y, p[1], p[3], p[5]], i3);
        }), cp.start.x, cp.start.y, p[0], p[1], p[2], p[3], p[4], p[5]);
      case "Q":
        return _Path.getPointOnQuadraticBezier(t2length(length, _Path.getPathLength(dataArray), (i3) => {
          return getQuadraticArcLength([cp.start.x, p[0], p[2]], [cp.start.y, p[1], p[3]], i3);
  ...
  static calcLength(x, y2, cmd, points) {
    ...
    switch (cmd) {
      ...
      case "C":
        return getCubicArcLength([x, points[0], points[2], points[4]], [y2, points[1], points[3], points[5]], 1);
      case "Q":
        return getQuadraticArcLength([x, points[0], points[2]], [y2, points[1], points[3]], 1);
      case "A":
        len = 0;
...
var Transformer = class extends Group {
  ...
  _handleMouseMove(e) {
    ...
    if (this._movingAnchorName === "rotater") {
      ...
      let delta = Math.atan2(-y2, x) + Math.PI / 2 - rotateAnchorAngleRad;
      if (attrs.height < 0) {
        delta -= Math.PI;
      }
...
var Emboss = function(imageData) {
  ...
  for (let p = 0, i2 = 0; i2 < data.length; i2 += 4, p++) {
    lum[p] = 0.2126 * src[i2] + 0.7152 * src[i2 + 1] + 0.0722 * src[i2 + 2];
  }
...
var _a;
var D = ((_a = lib_default.default) == null ? void 0 : _a.Stage) || lib_default.Stage;
...
var u = lib_default.default || lib_default;
var X = i("Arc", u.Arc);
